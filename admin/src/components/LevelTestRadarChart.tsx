...
// 오각형 레이더 차트로 보여준다. 다섯 값을 모두 받아야 하고(호출부에서 하나라도 비어
// 있으면 아예 렌더링하지 않는다), 순수 SVG라 별도 차트 라이브러리 없이 서버 컴포넌트
// 안에서도 그대로 쓸 수 있다.
...
// 결과 화면 상단에 함께 쓰는 범례 — 리스트 위쪽에서 각 영역이 어떤 색인지 먼저
// 보여준다(레이더 차트의 점 색상과 동일하게 맞춘다). vertical이면 차트 옆에 세로로
// 나란히 두는 성장 리포트 레이아웃용, 아니면 가로로 줄바꿈되는 기본형.
