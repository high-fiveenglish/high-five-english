...
// Encouragement) 구조로 파싱해 영역별 색상 카드로 나눠 보여준다. 강사는 영어
// 포털에서 이 8개 표제를 그대로 문단 제목으로 입력하는 것을 전제로 한다 — 표제가
// 안 보이면 그 문단은 통째로 "Teacher Feedback" 아래 본문으로 들어가고, 인식 못한
// 줄 구성(Student:/Teacher: 짝이 안 맞는 경우 등)도 평문으로만 표시될 뿐 깨지지
// 않는다(EvaluationContent와 같은 설계 원칙).
...
export function LevelTestReportContent({ content }: { content: string }) {
