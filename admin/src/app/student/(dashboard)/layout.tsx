...
import { requireStudent, isImpersonating } from "@/lib/studentAuth";
...
export default async function StudentDashboardLayout({ children }: { children: ReactNode }) {
  const [student, impersonating] = await Promise.all([requireStudent(), isImpersonating()]);

  return (
    <div className="min-h-screen bg-slate-100">
      {impersonating && <ImpersonationBanner studentName={student.name} />}
