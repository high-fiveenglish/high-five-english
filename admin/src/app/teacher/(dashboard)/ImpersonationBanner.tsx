import { endTeacherImpersonation } from "@/app/(admin)/teachers/actions";
...
export function ImpersonationBanner({ teacherName }: { teacherName: string }) {
  return (
    <div className="flex items-center justify-between gap-3 bg-amber-400 px-6 py-2 text-sm text-amber-950">
      <span>
        관리자 대리 로그인 중 — 현재 <strong>{teacherName}</strong> 강사 계정으로 보고 있습니다.
      </span>
      <form action={endTeacherImpersonation}>
        <button
          type="submit"
